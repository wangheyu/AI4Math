...
\begin{tikzpicture}[scale=0.9]
  ...
  \draw[thick,->] (2,0) -- (3,-2);
  \draw[thick,->] (3,-2) -- (4,0);
  \filldraw[fill=blue!30,draw=blue] (0,0) circle (4pt);
  \filldraw[fill=blue!30,draw=blue] (2,0) circle (4pt);
  \filldraw[fill=blue!30,draw=blue] (4,0) circle (4pt);
  \filldraw[fill=red!30,draw=red]   (1,-2) circle (4pt);
  \filldraw[fill=red!30,draw=red]   (3,-2) circle (4pt);
  \node[font=\small] at (0.5,-0.4) {光滑};
  \node[font=\small] at (1.5,-0.3) {限制};
  \node[font=\small] at (1.0,-1.4) {求解};
  \node[font=\small] at (2.5,-0.4) {延拓};
  \node[font=\small] at (3.5,-0.4) {光滑};
  \node[font=\small] at (0,0.4) {细网格};
  \node[font=\small] at (2,0.4) {细网格};
  \node[font=\small] at (4,0.4) {细网格};
  \node[font=\small] at (1,-2.4) {粗网格};
  \node[font=\small] at (3,-2.4) {粗网格};
  \node[font=\normalsize\bfseries] at (2,-3.2) {V-cycle: 递归得 $O(n)$ 最优复杂度};
\end{tikzpicture}
\end{center}
\end{frame}
