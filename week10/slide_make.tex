...
%------------------------------------------------
\begin{frame}[fragile,shrink]{一个真实例子：编译课程报告}
上周的课程报告 \texttt{report.pdf}，手工生成需要以下步骤：

\textbf{第一步：创建目录}

\begin{lstlisting}[language=bash]
mkdir -p build/mplconfig figures
mkdir -p output/jpl_compare_output build/chapters
\end{lstlisting}

\textbf{第二步：运行 Python 脚本生成图表和数据}

\begin{lstlisting}[language=bash]
conda run -n Teaching python src/Euler_vs_Verlet.py
conda run -n Teaching python src/JPL.py
\end{lstlisting}

\textbf{第三步：编译 LaTeX（含参考文献，需多轮）}
