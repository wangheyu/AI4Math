...
\midrule
Euler & $10^{-2}$ & $2.2556$ & $3.48\times 10^{-1}$ & $2.27\times 10^{-1}$ & 轨道明显发散 \\
Vel-Verlet & $10^{-2}$ & $8.26\times 10^{-3}$ & $4.22\times 10^{-12}$ & $1.41\times 10^{-16}$ & 仍接近圆轨道 \\
Euler & $10^{-3}$ & $3.59\times 10^{-1}$ & $6.84\times 10^{-2}$ & $3.60\times 10^{-2}$ & 漂移仍明显 \\
Vel-Verlet & $10^{-3}$ & $8.27\times 10^{-5}$ & $5.40\times 10^{-15}$ & $2.69\times 10^{-15}$ & 结果极稳定 \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figures/circle_orbit_comparison.png}
\caption{圆轨道上一周期内的数值轨道对比。左图对应步长 \(h=10^{-2}\)，右图对应步长 \(h=10^{-3}\)。黑色曲线为理论圆轨道，红色曲线为 Euler 方法结果，蓝色曲线为 Velocity-Verlet 方法结果，橙色点表示原点处的太阳。Euler 方法在较大步长下出现明显漂移，而 Velocity-Verlet 方法仍能较好保持圆轨道结构；当步长减小到 \(10^{-3}\) 时，两者误差均减小，但后者仍更稳定。}
\label{fig:circle-orbit-comparison}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figures/conservation_errors_h1e-02.png}
\caption{步长 \(h=10^{-2}\) 时的守恒量误差对比。左图为相对能量误差，右图为相对角动量误差。红色曲线对应 Euler 方法，蓝色曲线对应 Velocity-Verlet 方法。Euler 方法的误差会随时间迅速累积，而 Velocity-Verlet 方法在同样步长下仍能把误差控制在较小范围内，说明其更适合保守轨道问题。}
\label{fig:conservation-errors-h1e-2}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figures/conservation_errors_h1e-03.png}
\caption{步长 \(h=10^{-3}\) 时的守恒量误差对比。左图为相对能量误差，右图为相对角动量误差。与 \(h=10^{-2}\) 相比，两种方法的误差都进一步降低；但 Euler 方法仍有明显累积，而 Velocity-Verlet 方法的误差始终接近机器精度。}
...
\]
\begin{align*}
\dot v_x&=-\mu \frac{x}{(x^2+y^2+z^2)^{3/2}},\\
\dot v_y&=-\mu \frac{y}{(x^2+y^2+z^2)^{3/2}},\\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{output/jpl_compare_output/sun_embary_compare_errors.png}
