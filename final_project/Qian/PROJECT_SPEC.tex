...
\begin{abstract}
\noindent 本项目要求学生在钱学森《星际航行概论》（2008 版）第 5--6 章建立的拼接圆锥曲线理论基础上，扩展求解一个完整的工程级行星际轨道问题：火箭从地球出发，借助月球引力做一次速度修正，进入日心椭圆转移轨道；绕过太阳后返回与地球相遇。学生需在 2026 年一年周期内搜索使总速度增量最小的发射窗口。项目融合理论建模、数值仿真、JPL Horizons 历表对比、参数优化与 AI 辅助开发五个层面，是对本课程主要技能的综合考核。
\end{abstract}
...
本项目要求在钱先生的理论基础上，借助现代数值方法和真实历表数据，\textbf{补齐上述三项扩展}。
...
\begin{enumerate}[label=\textbf{L\arabic*.},leftmargin=*]
    \item \textbf{理论建模}：将钱学森的拼接圆锥曲线法扩展到含月球借力的三段轨道；
    \item \textbf{数值仿真}：用合适的数值积分方法实现 N-体引力问题的稳定数值传播；
    \item \textbf{历表数据}：调用 JPL Horizons 系统获取真实天体星历，并与数值仿真对比；
    \item \textbf{优化算法}：在一年周期内做参数扫描与精化，求解最优发射窗口；
    \item \textbf{科学计算实践}：编写可重现、可验证、模块化的科学计算代码；
    \item \textbf{AI 辅助开发}：使用 Claude Code、Cursor 等 AI 工具辅助代码编写与调试，并如实记录；
    \item \textbf{学术写作}：用 LaTeX 撰写规范的学术报告，含公式、图表、文献引用。
\end{enumerate}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{积分器（推荐）}：Velocity-Verlet（2 阶辛积分器），主步长 $h = 3600$\,s，事件细化阶段可减至 $h = 60$\,s。学生可自行选择其他积分方案（如 RK4、辛 Yoshida 4 阶等），但须在报告中说明选择理由与守恒性验证；
...
学生必须完成以下各项。任何一项缺失或不达标，将\textbf{酌情扣分}。
...
\begin{description}[leftmargin=2.5em,style=nextline]
    ...
    \item[M3\,JPL Horizons 历表对照] 仅用 N-体传播 Sun-Earth-Moon 一年，与 JPL Horizons 历表对比，输出每日位置与速度残差，要求所有天体位置残差 $\le 1000$\,km。
    ...
    \item[M8\,可视化与展示] 生成静态图（轨道、能量守恒、扫描曲线、残差等）与一段 30--60 秒的轨道动画 MP4，含关键事件 zoom-in。
\end{description}
...
\begin{description}[leftmargin=2.5em,style=nextline]
    ...
    \item[O5\,实时交互演示] 仿照 \texttt{week9/src/two\_body\_realtime.py} 风格做一个 Tkinter 或 Web 实时交互演示。

    \item[O6\,其他创新] 任何与项目主题相关、有思考深度的扩展（如机器学习辅助轨道设计、并行计算加速扫描等）。
\end{description}

% ============================================================
\section{致谢要求}\label{sec:credits}

报告须包含独立的\textbf{致谢}章节，如实声明以下两类信息：

\subsection{他人帮助的声明}

虽然本项目是\textbf{个人项目，不允许团队合作}，但学生可以从同学、助教、网络论坛等渠道获得概念性讨论或调试建议。\textbf{凡获得他人帮助者必须在致谢中声明}，例如：
...
允许使用 AI 工具，但\textbf{必须如实声明使用了哪些工具、提供了什么帮助}。声明粒度需具体到模块或步骤，例如：
...
提交一个压缩包 \texttt{学号\_姓名\_FinalProject.zip}，根目录\textbf{仅含}以下内容（多余文件请勿提交）：
...
\begin{itemize}[leftmargin=*]
    \item \textbf{提交压缩包（make 前）}：$\le 50$\,MB；
...
\begin{notebox}[代理可用性]
代理 URL \texttt{http://8.216.49.176:18766/api/horizons.api}，token \texttt{fce2a741a94feded5c3b9b7ab51f6748}。2026 全年日期都在 EOP 覆盖范围内（实测数据至 2026-05-29，预报至 2026-08-24）。
\end{notebox}
...
逾期提交不予接收。建议预留至少 3 天进行最终编译测试与 \texttt{make all} 流程验证，避免临近 DDL 出现环境问题。
