  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \begin{block}{减少深度}
        ...
        \footnotesize
        \begin{itemize}
          \item $v(s)$：对局面 $s$ 的胜负估计。
          \item $v^*(s)$：双方完美下棋时的最优价值。
        \end{itemize}
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{减少宽度}
        ...
        \begin{itemize}
          ...
          \item $p(a\mid s)$：局面 $s$ 下选择动作 $a$ 的概率。
        \end{itemize}
        对应 AlphaGo 中的 \textbf{policy network}，目标是降低有效分支因子 $b$。
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{自我对弈与奖励信号}
  ...
  \begin{block}{什么是历史版本}
    $p_{\rho^-}$ 表示训练过程中保存下来的旧策略。当前网络每训练一段时间，就把一个冻结副本放入对手池；之后自我对弈时随机抽一个旧版本作为对手。
  \end{block}

  \begin{itemize}
    \item 对手不随当前网络同步变化，能减少策略振荡。
    \item 随机面对多个旧版本，避免只针对单一对手过拟合。
    \item 围棋中间没有即时奖励，只有终局胜负。
  \end{itemize}

  \footnotesize
  下标 $t$ 表示第 $t$ 手；$z_t$ 是从第 $t$ 手当前玩家视角看的最终结果。
  \normalsize
  \[
    z_t =
    \begin{cases}
...
\begin{frame}{价值网络为什么重要}
  若没有 value network，MCTS 常用 rollout 评估局面：

  \[
    \text{从当前局面快速模拟到终局，再看谁赢}
  \]

  但快速 rollout policy 较弱，终局结果噪声大。

  \begin{block}{价值网络的作用}
    用一次神经网络前向计算，直接估计局面胜负前景，减少搜索深度。
  \end{block}

  论文指出：单次 \valuefn 评估接近使用 RL policy rollout 的准确度，但计算量少约 $15000$ 倍。
\end{frame}
...
\begin{frame}{MCTS 四步}
  ...
  \begin{itemize}
    \item Selection：选择 $Q(s,a)+u(s,a)$ 最大的分支。（$u(s,a)$ 说明见下一页）
...
\begin{frame}{Selection：利用和探索的平衡}
  ...
  \begin{block}{Selection 的直观}
    从根局面开始，反复按上式选择分支，沿当前搜索树一直走到边界；这个边界局面就是本次模拟的叶节点。
  \end{block}
